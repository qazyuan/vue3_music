...
import {reactive, ref, toRefs, onMounted, set, defineComponent, Ref } from '@vue/composition-api';
...
function useSearchHistory() {
    ...
    const updateHistory = (word:string) => {
        ...
        if(state.searchHistory && state.searchHistory.length > 10) {
...
// 搜索结果
function  useSearchResult(searchType: Ref<number>, searchWord: Ref<string>, updateHistory: (word:string)=>void) {
    ...
    const handleToSearch = (name:string) => {
...
// 清楚搜索内容
function useSearchClear(searchType: Ref<number>, searchWord: Ref<string>) {
